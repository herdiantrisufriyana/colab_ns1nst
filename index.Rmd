...
This analysis used the first assumption. [**Click here **](https://herdiantrisufriyana.github.io/colab_ns1nst/index2.html) for the second assumption.
...
raw_data=
  read_xlsx('inst/extdata/raw_data.xlsx',skip=1)
```
...
raw_data %>%
  ...
  write_csv('inst/extdata/raw_data_colnames_old.csv')
```
...
raw_data2=
  ...
  `colnames<-`(
    colnames(.) %>%
      data.frame(colname_old=.) %>%
      left_join(
        read_csv('inst/extdata/raw_data_colnames_new.csv',show_col_types=F)
...
raw_data3$non_numeric %>%
  ...
  write_csv('inst/extdata/raw_data_non_numeric_values_old.csv')
```
  ...
  cbind(
    raw_data3$non_numeric %>%
      ...
      lapply(
        left_join
        ,read_csv(
          'inst/extdata/raw_data_non_numeric_values_new.csv'
          ,show_col_types=F
        )
...
Nevertheless, only 1 missing value for plt, which was considerably negligible.
...
Data distribution of the imputed variables were similar before and after imputation.
...
We included 27 variables for the downstream analyses, including strata, dependent, and independent variables.
...
correlation_direction=
  read_csv('inst/extdata/correlation_direction.csv',show_col_types=F)
```
...
correlation_to_indep_sig  %>%
  filter(to%in%c(var$independent)) %>%
...
We adjust the effects of independent variables and other variables using their confounders.
...
We adjust the effects of variables of interest, i.e., independent variables and other variables that are significant in the multivariate regressiong analysis, using their confounders and mediators.
...
Since the sample size only allowed for 1 covariate in addition to variables of interest, we reduced the number of covariates without leaking information of the dependent variable as the outcome. Principal component analysis was conducted for this purpose.
...
All the covariates of variables of interest could be reduced into 1 covariate using PC1.

Now, we conducted mediation analysis whether the variables' effect on the dependent variable persisted without the mediators' effect.
...
There was no significant effect of `age` and `sex` on the dependent variable beyond the `outcome` effect. In contrast, the effect of `ns1_od` on the dependent variable persisted without `outcome`. There was no mediator between `aptt` and the dependent variable.

# Conclusion

The effect of `ns1_od` on `nst_od` was mediated by `outcome`.
