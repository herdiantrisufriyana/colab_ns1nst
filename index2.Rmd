...
This analysis used the second assumption. [**Click here **](https://herdiantrisufriyana.github.io/colab_ns1nst/index.html) for the first assumption.
...
raw_data=
  read_xlsx('inst/extdata2/raw_data.xlsx',skip=1)
```
...
raw_data %>%
  ...
  write_csv('inst/extdata2/raw_data_colnames_old.csv')
```
...
raw_data2=
  ...
  `colnames<-`(
    colnames(.) %>%
      data.frame(colname_old=.) %>%
      left_join(
        read_csv('inst/extdata2/raw_data_colnames_new.csv',show_col_types=F)
...
raw_data3$non_numeric %>%
  ...
  write_csv('inst/extdata2/raw_data_non_numeric_values_old.csv')
```
  ...
  cbind(
    raw_data3$non_numeric %>%
      ...
      lapply(
        left_join
        ,read_csv(
          'inst/extdata2/raw_data_non_numeric_values_new.csv'
          ,show_col_types=F
        )
...
var_num_non_normal_after_trans %>%
  data.frame(variable=.) %>%
  write_csv('inst/extdata2/var_num_non_normal_after_trans.csv')
```

```{r Define cats for vars that are not normal after transformed, include=FALSE}
var_num_non_normal_after_trans_category=
  read_csv(
    'inst/extdata2/var_num_non_normal_after_trans_category.csv'
    ,show_col_types=F
  )
```
...
pair_results %>%
  ...
  arrange(Var1,Var2) %>%
  write_csv('inst/extdata2/correlation.csv')
```
...
correlation_direction=
  read_csv('inst/extdata2/correlation_direction.csv',show_col_types=F)
```
